...
func (chn *Channel) handlemsg(msgType int, msg []byte) {
	...
	client, err := rpc.DialHTTP("tcp", "127.0.0.1:1213")
	if err != nil {
		log.Fatal("dialing: ", err)
	}
	args := &ClientInfo{chn.id, chn.rm.name, msg}
	var reply string
	err = client.Call("Recevier.Handler", args, &reply)
	if err != nil {
		log.Fatal("rpc error: ", err)
	}
	fmt.Printf("rpc reply:%v", reply)
